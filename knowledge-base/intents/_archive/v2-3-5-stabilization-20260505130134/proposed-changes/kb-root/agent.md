...
- **Scope:** Maintain template `@williamduong/kb` + build roadmap v1.3 → v3.0
...
1. Chạy `kb intent list` và đọc `intent.md` của các intent active để lấy summary ngắn.
...
```
v1.3 Git Binding + kb status unified entry  (target tới đây trước)
...
- Sau task nghiêm túc, kết bằng:
  ...
  - **Handoff:** bảng owner routing — HUMAN / CLI / @SVFactory / @kb (bỏ qua nếu task trivial)
