...
3. CLI deterministic behavior (`kbx status`, `kbx intent`, `kbx maintain`, `kbx release`) can be validated in both environments, but prompt/persona behavior must be accepted only in the target persona environment.
...
Before creating any new intent or starting work on a new version, the agent MUST run two gates in order. These gates apply to both `@kbx` (KB Agent) and `@sfact` (maintainer agent).
...
- Source template: `template/.github/agents/kbx.agent.template.md`
...
- When changing agent doctrine or capability language in this manual, review `template/.github/agents/kbx.agent.template.md` in the same wave so init projections stay aligned.
