...
Anchor intent:
- `v2-9-kbagent-observability-graph`

Primary objective:
- Prepare a disciplined execution path before opening multi-week UI implementation.
...
- Multiple active intents currently coexist; roadmap work should remain anchored to one active execution owner.
...
Entry gate:
- Active anchor intent metadata is complete (focus, next_action, decision_summary).

Exit gate:
- Roadmap language aligns with current runtime terminology.
- Lifecycle references are domain-explicit where needed.
