...
- [ ] step-2 (status: pending) — custom:close-v2-3-6-intent — Run `kb intent apply v2-3-6-planned-backlog` to finalize the governance change (Workflow 7 focus.md step). The staged `svfactory/process.md` was already committed manually; apply closes the intent record.
...
- `v2-3-6-planned-backlog` staged `svfactory/process.md` — change committed to git but `kb intent apply` not yet executed.
