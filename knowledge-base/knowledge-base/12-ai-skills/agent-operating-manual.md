...
- **Include a `Handoff` table** at the end of any substantial task output. This table makes pending coordination tasks explicit and routes them to the correct owner.
...
- A `Blocking? = Yes` item must appear first or be called out explicitly.
- Tasks with `Owner = CLI` must include the exact command in the task cell.
