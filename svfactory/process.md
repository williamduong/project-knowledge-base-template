...
10. **Downstream Acceptance Gate** (bắt buộc khi `change_scope` chứa `template/`):
    - Nếu intent chạm bất kỳ file nào trong `template/` (ship surface), phải thực hiện:
      ...
      4. Mở KB Agent (`@kbx`) trong downstream workspace, hỏi câu hỏi liên quan đến thay đổi, verify response đúng hướng
...
7. **Kết thúc session — bắt buộc update `focus.md`** (P1: docs phải theo code):
   ...
   - Commit `svfactory/focus.md` vào git trước khi kết thúc session
